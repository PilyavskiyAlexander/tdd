...
class ParticipateInFormTest extends TestCase
{
    ...
    {
//        $this->expectException(); //TODO Должна выбиватся ошибка из-за user, который не аутентифицированный, но ее почемуто нет
        $user = factory('App\User')->create();
        $thread = factory('App\Thread')->create();
        $reply = factory('App\Reply')->create(['user_id' => $user->id, 'thread_id' => $thread->id]);

        $this->post(route('thread_repliers', $thread), $reply->toArray());
